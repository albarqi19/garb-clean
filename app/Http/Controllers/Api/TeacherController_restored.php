...
class TeacherController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        try {
            $query = Teacher::with([
                'mosque:id,name',
                'quranCircle:id,name,grade_level,is_active'
            ]);
            ...
            $teachers->getCollection()->transform(function ($teacher) {
                ...
                return [
                    ...
                    'الحلقة' => $teacher->quranCircle ? [
                        ...
                        'اسم' => $teacher->quranCircle->name,
                        'المستوى' => $teacher->quranCircle->grade_level ?? 'غير محدد'
                    ] : null,
    ...
    public function show($id): JsonResponse
    {
        try {
            $teacher = Teacher::with([
                ...
                'quranCircle:id,name,grade_level',
                'attendances' => function ($query) {
                    $query->orderBy('date', 'desc')->limit(30);
                },
                'incentives' => function ($query) {
                    $query->orderBy('date', 'desc')->limit(10);
                },
                'salaries' => function ($query) {
                    $query->orderBy('month', 'desc')->limit(6);
                }
            ])->find($id);
            ...
            // إحصائيات جلسات التسميع للحلقة الخاصة بالمعلم
            ...
            $data = [
                ...
                'سجل_الحضور_الأخير' => $teacher->attendances->map(function ($attendance) {
                    return [
                        'التاريخ' => $attendance->date->format('Y-m-d'),
                        'وقت_الوصول' => $attendance->arrival_time?->format('H:i'),
                        'وقت_المغادرة' => $attendance->departure_time?->format('H:i'),
                        'حاضر' => $attendance->is_present ? 'نعم' : 'لا',
                        'ملاحظات' => $attendance->notes ?? 'لا توجد ملاحظات'
                    ];
                }),
                'الحوافز_الأخيرة' => $teacher->incentives->map(function ($incentive) {
                    return [
                ...
                'الرواتب_الأخيرة' => $teacher->salaries->map(function ($salary) {
                    return [
    ...
    public function teacherStats($id): JsonResponse
    {
        try {
            ...
            $recitationStats = RecitationSession::whereHas('student', function ($query) use ($teacher) {
                ...
                AVG(quality_score) as avg_quality,
                SUM(pages_recited) as total_pages,
                COUNT(CASE WHEN quality_score >= 80 THEN 1 END) as excellent_sessions
            ')->first();
        ...
        try {
            ...
            $attendances->getCollection()->transform(function ($attendance) {
                return [
                    ...
                    'وقت_الوصول' => $attendance->arrival_time?->format('H:i'),
                    'وقت_المغادرة' => $attendance->departure_time?->format('H:i'),
                    'مدة_الحضور' => $attendance->arrival_time && $attendance->departure_time 
    ...
    public function getTeacherStudents($id): JsonResponse
    {
        try {
            ...
            // 1. جلب الطلاب من الحلقة الأساسية للمعلم
            if ($teacher->quranCircle) {
                $mainCircleStudents = Student::where('quran_circle_id', $teacher->quranCircle->id)
                    ->with([
                        ...
                        'quranCircle.mosque:id,name,neighborhood,street',
                        'circleGroup:id,name',
                        'curriculum:student_id,current_surah,current_ayah,memorized_pages',
                ...
                foreach ($mainCircleStudents as $student) {
                    $allStudents->push([
                        ...
                        'حلقة' => [
                            ...
                            'المستوى' => $student->quranCircle->grade_level,
                            'الحلقة_الفرعية' => $student->circleGroup ? $student->circleGroup->name : null,
                            'نوع_التكليف' => 'أساسي'
                        ],
                        ...
                        // المنهج الحالي
                        'منهج_حالي' => [
                            'السورة_الحالية' => $student->curriculum->current_surah ?? 'غير محدد',
                            'الآية_الحالية' => $student->curriculum->current_ayah ?? 0,
                            'عدد_الصفحات_المحفوظة' => $student->curriculum->memorized_pages ?? 0,
                        ],
            ...
            foreach ($activeAssignments as $assignment) {
                ...
                $assignmentStudents = Student::where('quran_circle_id', $circle->id)
                    ->with([
                        'quranCircle:id,name,grade_level,circle_type,mosque_id',
                        'quranCircle.mosque:id,name,neighborhood,street',
                        'circleGroup:id,name',
