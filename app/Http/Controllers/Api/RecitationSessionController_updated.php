...
class RecitationSessionController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        try {
            ...
            if (!isset($sessionData['curriculum_id'])) {
                $studentProgress = StudentProgress::where('student_id', $request->student_id)
    ...
    public function update(Request $request, string $sessionId): JsonResponse
    {
        ...
        try {
            ...
            $session->update($request->only(['grade', 'notes', 'has_errors', 'correction_notes', 'duration_minutes', 'evaluation', 'status']));

            // إذا تم تغيير الحالة إلى "مكتملة"، قم بتحديث تقدم الطالب
            if ($request->has('status') && $request->status === 'مكتملة' && $oldStatus !== 'مكتملة') {
