                ...
                Forms\Components\Section::make('معلومات الجلسة')
                    ->schema([
                        Forms\Components\Select::make('circle_id')
                            ...
                            ->searchable()
                            ->default(function () {
                                // Get student's circle if available
                                $student = $this->getOwnerRecord();
                                return $student->circle_id ?? null;
                            }),
        ...
        return $table
            ...
            ->filters([
                ...
                Tables\Filters\Filter::make('recent')
                    ->label('الجلسات الحديثة')
                    ->query(fn (Builder $query): Builder => $query->where('session_date', '>=', now()->subDays(30))),
            ])
            ->headerActions([
